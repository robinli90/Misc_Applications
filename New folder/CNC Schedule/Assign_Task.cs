...
namespace CNC_Schedule
{
    public partial class Assign_Task : Form
    {
        ...
        public Assign_Task(CNC_Form _s, string _ordernumber, bool assigned, string _part)
        {
            s = _s;
            part = _part;
            InitializeComponent();
        ...
        {
            string unassign_query = "insert into d_cadcamjobassignlist values ('" + ordernumber + "', '" + selected_employee_number + "', 'CNC', '" + DateTime.Now + "', NULL, '" + part + "')";
            ExcoODBC database = ExcoODBC.Instance;
            OdbcDataReader reader;
            database.Open(Database.DECADE_MARKHAM);
            reader = database.RunQuery(unassign_query);
