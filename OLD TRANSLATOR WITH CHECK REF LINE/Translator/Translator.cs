 ...
 * Contact Robin (author) at rli@etsdies.com/lirobin9@gmail.com for inquiries into editing/compiling/adding to this code IF neccessary 
 * 
...
namespace Translator
{
    public partial class Translator : Form
    {
        ...
        private string Process_File_Output()
        {
            int iz = 0;
            string line = "";
            ...
            try
            {
                if (action == "Multiply" || action == "Divide" || action == "Add" || action == "Subtract" || action == "Set")
                {
                    foreach (char c in original)
                    {
                        ...
                        else if (parsing && c.ToString() == " ") //When end of dimension, set middle to number and perform arithmetic
                        {
                            if (action == "Multiply") middle = temp[0] + Math.Round((Convert.ToDouble(temp.Substring(1, temp.Length - 1)) * Convert.ToDouble(value)), 4).ToString();
                            if (action == "Divide") middle = temp[0] + Math.Round((Convert.ToDouble(temp.Substring(1, temp.Length - 1)) / Convert.ToDouble(value)), 4).ToString();
                            if (action == "Add") middle = temp[0] + Math.Round((Convert.ToDouble(temp.Substring(1, temp.Length - 1)) + Convert.ToDouble(value)), 4).ToString();
                            if (action == "Subtract") middle = temp[0] + Math.Round((Convert.ToDouble(temp.Substring(1, temp.Length - 1)) - Convert.ToDouble(value)), 4).ToString();
                    ...
                    G_Code_List[index] = front + (middle.StartsWith(".") ? "" : middle) + " " + back;
                    Change_Count++;
                }
                ...
                else if (action == "StoreValue")
                {
                    if (dimension.Length > 0)
                    {
                        ...
                        {
                            MEMORY_BIN[Convert.ToInt32(value)] = index.ToString();
                            STORED_COUNT++;
                        }
                        ...
                        if (optional_value_2.Length > 0 && !dimension.Contains("@!")) //if has rule to perform arithmetic on storage bin
                        {
                            ...
                            {
                                //Console.WriteLine(e);
                                MEMORY_BIN[Convert.ToInt32(value)] = middle;
                            }
                        }
                ...
                else if (action == "COMPARE_TO_REF_LINE")
                {
                    ...
                    {
                        if (G_Code_List[reference_index].Contains(temp2[0]))
                        {
        ...
        // Execute the appropriate changes to the G-Code; the changes are dynamic so changes that happened can/may change again due to other rules
            ...
            foreach (List<string> rules in _temp) // For each rule in rule_list, iterate through the G_Code_List 
            {
                ...
                foreach (string G_Code_Line in temp_G_Code.ToList())
                {
                    if (!line_action_check)
                    {
                        ...
                        if (rule[5].Contains("[/SFL]") && rule[5].Contains("[SFL]") && parameters_valid)
                        {
                            parameters_valid = false;

                            int start_index = rule[5].IndexOf("[SFL]") + 5;
                            int end_index = rule[5].IndexOf("[/SFL]");
                            string center = rule[5].Substring(start_index, end_index - start_index);
                            if (i >= Convert.ToDouble(GET_TEXT_WITH_BIN(center)))// line number greater or equal to the center line value comparison
                            {
                                parameters_valid = true;
                            }
                        }
                        if (parameters_valid)
                        {
                            ...
                            if (rule[0] == "RunQuery" || rule[0] == "GET_INPUT" || rule[0] == "DeleteAt" || rule[0] == "InsertAt" || rule[0] == "Switch" || rule[0] == "ReplaceAt" || rule[0].Contains("LastIndexOfReplace") || rule[0].Contains("IF_THEN") || rule[5].Contains("FIRST_INSTANCE_ONLY") || rule[0].Contains("RANGE_FUNCTION")) // Do these only once
