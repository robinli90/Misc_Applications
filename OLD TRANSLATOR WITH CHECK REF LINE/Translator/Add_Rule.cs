...
namespace Translator
{
    public partial class Add_Rule : Form
    {
        ...
        public Add_Rule(Translator main, Point spawn_location_, bool _EDIT = false, int index = 0, string A = "", string B = "", string C = "", string D = "", string E = "", string F = "", List<int> All_Selected_Indicies = null)
        {
            ...
            condition_direction.Items.Add("Greater Than"); condition_direction.Items.Add("Less Than"); condition_direction.Items.Add("Equal To");
        ...
        {
            if ((ACTION.Contains("?") || SEARCH_VALUE.Contains("?") || DIMENSION.Contains("?") || VALUE.Contains("?") || OPTIONAL_VALUE.Contains("?") || COMMENT.Contains("?")))
            {
        ...
        private void Perform_Save_Rule(int index=-1)
        {
            ...
            if (start_from_line.Checked && start_from_line_value.Text.Length > 0 && !_GROUP_TEXT.Contains("[SFL]"))
            {
                _GROUP_TEXT += "[SFL]" + start_from_line_value.Text + "[/SFL]" + comment_box.Text;
            }
            ...
            else if (check_boxes[21].Checked && query_box.Text.Length > 0)
            {
                ...
                {
                    Add_Rule_Check("RunQuery", "", enc_obj.Encrypt(query_box.Text), "", "", comment_box.Text, index);
                }
            }
        ...
        private void Edit_Parse()
        {
            ...
            */
            ...
            if (_COMMENT.Contains("[/SFL]"))
            {
                int start_index = _COMMENT.IndexOf("[SFL]") + 5;
                int end_index = _COMMENT.IndexOf("[/SFL]") + 6;
                start_from_line_value.Text = _COMMENT.Substring(start_index, end_index - start_index - 6);
                _GROUP_TEXT += _COMMENT.Substring(start_index - 5, end_index - start_index + 5);
                _COMMENT = _COMMENT.Substring(end_index);
                start_from_line.Checked = true;
                start_from_line.Enabled = false;
                start_from_line_value.BackColor = Color.LightYellow;
                start_from_line_value.Enabled = true;
            }
